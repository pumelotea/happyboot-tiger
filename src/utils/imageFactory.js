...
export function watermark (
  callback, image, sign, left = 0, top = 0, imageWidth = '', imageHeight = '', signWidth = '', signHeight = ''
) {
  if (!image || !sign) {
    callback({
      success: false,
    ...
    // 绘制底图
    ctx.drawImage(
      image, 0, 0, imageSize.w, imageSize.h
    )
    // 绘制水印
    ctx.drawImage(
      sign, left, top, signSize.w, signSize.h
    )
    // 返回base64
    callback({
      success: true,
...
export function cropper (
  callback, image, left = 0, right = 0, top = 0, bottom = 0, width = 0, height = 0
) {
  if (!image) {
    callback({
      success: false,
    ...
    // 裁剪
    const clip = ctx.getImageData(
      left, top, canvasSize.w, canvasSize.h
    )
    // 调整画布大小
...
export function resize (
  callback, image, width = 0, height = 0
) {
  if (!image) {
    callback({
      success: false,
